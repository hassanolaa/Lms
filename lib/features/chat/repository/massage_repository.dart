...
class massage_repository {
  ...
  static Future<void> uploadrecord(
      String chatid, String path, int platform, {Uint8List? file}) async {
    print("in upload record");
    if (platform == 0) {
            try {
    print("in upload record222");
        final uuid = Uuid().v4();
        Reference ref = await FirebaseStorage.instance
            .ref()
            .child("records")
    ...
    print("in upload record333");

        if(kIsWeb){
        var uploadTask = await ref.putData(file!, metadata);
        print("Uploaded video to server");
